...
public class AuthHelper {
			...
			if (cookie != null && !cookie.equals("")) {
				...
				if (cookie != null && !cookie.equals("")) {
					...
					if (isWeiXinAPPAccess(request)) {// 微信登录
						if (cookies.length > 1) {
							// 根据cookie中的帐号查询用户
							user = iUser.userRightByUserId(new Integer(cookies[0]));
						}
					} else {
						if (cookies.length == 3) {
	...
	public static void setLoginUser(HttpServletRequest request, HttpServletResponse response, User user)
		...
		if (user != null) {
			...
			// WebHelper.setCookie(response, "zhanglin-com", cookieValue, 360 * 24 * 60 *
			// 60, webProperties.getCookieDomain(),
			// null);
			WebHelper.setCookie(response, webProperties.getCookieName(), cookieValue, 60 * 60,
					webProperties.getCookieDomain(), null);
	...
	public static void rememberMe(HttpServletRequest request, HttpServletResponse response, String account) {
		try {
			...
			if (StringUtils.isNotEmpty(rememberMe) && StringUtils.isNotEmpty(account)
					&& "rememberMe".equals(rememberMe)) {
				WebHelper.setCookie(response, "rememberMe",
						SecurityHelper.desEncrypt(account, webProperties.getSecurityKey()));
			} else {
